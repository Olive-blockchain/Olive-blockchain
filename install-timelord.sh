...
THE_PATH=$(python -c 'import pkg_resources; print( pkg_resources.get_distribution("olivevdf").location)' 2>/dev/null)/vdf_client
OLIVEVDF_VERSION=$(python -c 'from setup import dependencies; t = [_ for _ in dependencies if _.startswith("olivevdf")][0]; print(t)')
...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		echo "Installing olivevdf from source on Ubuntu/Debian"
		...
		echo venv/bin/python -m pip install --force --no-binary olivevdf "$OLIVEVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary olivevdf "$OLIVEVDF_VERSION"
		symlink_vdf_bench "$PYTHON_VERSION"
	elif [ -e venv/bin/python ] && test $RHEL_BASED; then
		echo "Installing olivevdf from source on RedHat/CentOS/Fedora"
		# Install remaining needed development tools - assumes venv and prior run of install.sh
		...
		echo "Installing olivevdf requirement boost for MacOS."
		brew install boost
		echo "Installing olivevdf from source."
