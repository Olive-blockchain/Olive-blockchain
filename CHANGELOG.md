...
- Olivelisp programs now remained serialized in Node for better performance.
...
- This is the 1.0 release of Olivelisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [olivelisp.com](https://olivelisp.com) shortly.
...
- You should review the [release notes for v1.0.2](https://github.com/Olive-Network/olive-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.
...
- We have released version 1.0.0 of [olivepos](https://github.com/Olive-Network/olivepos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/Olive-Network/olive-blockchain/issues/1189)
...
- Generators have been re-worked in CLVM. We added a olivelisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during OliveLisp native deserialization.
...
- Olive wheels not on Pypi and some dependecies not found there also are now on pypi.olive.net.
...
- Olivelisp and the clvm have had the standard puzzle updated and we replaced `((c P A))` with `(a P A)`.
...
- calccrypto/uint128_t was updated in the Windows olivepos implementation. Olivepos required some changes its build process to support MacOS ARM64.
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/Olive-Network/olive-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `olive -h` and `olive SUBCOMMAND -h` can be your guide.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Olive-Network/olive-blockchain/releases/tag/1.0rc1) was well.
...
- There is now remote UI support. [Documents](https://github.com/Olive-Network/olive-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/Olive-Network/olive-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- The GUI is now in a separate submodule repository from olive-blockchain, [olive-blockchain-gui](https://github.com/Olive-Network/olive-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `olive-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/Olive-Network/olive-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
- Sync store now tracks all connected peers and removes them as they get removed.
- The Rate Limited Wallet has been ported to new consensus and updated Olivelisp methods.
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/Olive-Network/olive-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Olive-Network/olive-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- clvm bytecode is now generated and confirmed that the checked-in clvm and OliveLisp code matches the CI compiled code.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [olive.net](https://olive.net/). Among the improvements this gives the Olive blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
