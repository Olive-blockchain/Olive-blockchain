...
- Migration with COVID_ROOT set does not crash olive init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TCOV Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.olive.pinksheetscrypto.com/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- We have moved to a new and much higher scalability download.olive.pinksheetscrypto.com to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.olive.pinksheetscrypto.com/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.olive.pinksheetscrypto.com/latest/Setup-MacOS.dmg).
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.olive.pinksheetscrypto.com/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.olive.pinksheetscrypto.com via https, for example.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [Covid Blockchain GUI](https://crowdin.com/project/olive-blockchain) there.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [olive.pinksheetscrypto.com](https://olive.pinksheetscrypto.com/). Among the improvements this gives the Covid blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Covid Network at backup.olive.pinksheetscrypto.com. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Covid backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.olive.pinksheetscrypto.com, your own installation, or a third party's version of it.
...
- Covid start commands like `olive start farmer` and `olive stop node` now keep track of process IDs in a run/ directory in your configuration directory. `olive stop` is unlikely to work on Windows native for now. If `olive start -r node` doesn't work you can force the run/ directory to be reset with `olive start -f node`.
...
- There is a new command `olive init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `olive-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/pinksheetscrypto/olive-blockchain/wiki/Updating-beta-software). If you've set `$COVID_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- Configuration directories have been moved to a default location of HomeDirectory/.olive/release/config, plots/ db/, wallet/ etc. This can be overridden by `export COVID_ROOT=~/.olive` for example which would then put the plots directory in `HomeDirectory/.olive/plots`.
...
- Covid fullnode, farmer and harvester now default to logging to olive.log in the olive-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- More details on the release at [https://www.olive.pinksheetscrypto.com/developer/](https://www.olive.pinksheetscrypto.com/developer/)
