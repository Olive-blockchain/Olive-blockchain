...
- We added a simple profiler to track performance times in the application (see /olive/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `olive configure -t true|false`).
...
- We now use version 1.0.2 of olivepos. We now reopen files on read or write failure with thanks to @mreid-moz! We also prevent olivepos prover crashing for more bad plots.
...
- Secret wallet key is hidden by default in `olive keys show`. Use `olive keys show --show-mnemonic-seed` for private keys.
...
- We changed olivelisp singletons to take a puzzlehash as its origin. We also updated the DID wallet to use this.
...
- This is the 1.0 release of Chialisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [olivelisp.com](https://olivelisp.com) shortly.
...
- The `src` directory in olive-blockchain has been changed to `olive` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `olive/`
...
- You should review the [release notes for v1.0.2](https://github.com/Chia-Network/olive-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `olive keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of olivepos 1.0.0 as well.

## 1.0.2 Chia Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [olivepos](https://github.com/Chia-Network/olivepos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/olive-blockchain).
...
- @jespino changed `olive keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `olive keys show`.
...
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `olive keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/olive-blockchain/).
...
- Users can now pip install e.g. olive-blockchain==1.0.1 on most platforms.
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/olive-blockchain).
...
- This is a hard fork/breaking change from RC6/7. TXCH Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.olive.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `olive farm summary` will now display TXCH or XCH as appropriate.
...
- `olive farm summary` displayed some incorrect amounts.
...
- `olive netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `olive netspace`. Thanks @altendky.
- There was a typo in `olive plots`. Thanks @adamfiddler.

### Known Issues

- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/Chia-Network/olive-blockchain/issues/1189)
...
- Remove `olive plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `olive keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
- Version 1.0.1 of olivevdf was added. This brought MPIR on Windows to the most recent release. Additionally we removed inefficient ConvertIntegerToBytes() and ConvertBytesToInt() functions, use GMP library's mpz_export/mpz_import for big integers and simple helper functions for built-in integer types. The latter are taken from olivevdf. We now require compressed forms to be encoded canonically when deserializing. This should prevent potential grinding attacks where some non-canonical encodings of a compressed form could be used to change its hash and thus the next challenges derived from it. Canonically encoded compressed forms must be reduced and must produce the same string when deserialized and serialized again.
...
- We now hand build Apple Silicon native binary wheels for all olive-blockchain dependencies and host them at [https://pypi.olive.net/simple](https://pypi.olive.net/simple). We are likely to hand build a MacOS ARM64 dmg available and certainly will for 1.0. You can install natively on M1 now with the `git clone` developer method today. Just make sure Python 3.9 is installed. `python3 --version` works.
...
- Generators have been re-worked in CLVM. We added a olivelisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during ChiaLisp native deserialization.
...
- We have moved to a new and much higher scalability download.olive.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.olive.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.olive.net/latest/Setup-MacOS.dmg).
- Chia wheels not on Pypi and some dependecies not found there also are now on pypi.olive.net.
...
- PRs and commits to the olive-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TXCH farmed.
- `olive farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `olive wallet` had various bugs.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.olive.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `olive farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/olive-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `olive wallet` now takes get_address to get a new wallet receive address from the CLI.
- `olive plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of olive-blockchain, etc., instead of in blspy.
...
- In olivevdf we changed n-Wesolowski proofs to include B instead of y in segments. Proof segments now have the form (iters, B, proof) instead of (iters, y, proof). This reduces proof segment size from 208 to 141 bytes.
- The new olivevdf proof format is not compatible with the old one, however zero-Wesolowski proofs are not affected as they have zero proof segments and consist only of (y, proof).
- We made two HashPrime optimizations in olivevdf. This forces numbers being tested for primality to be odd and avoids an unnecessary update of the sprout vector by stopping after the first non-zero value. This is a breaking change as it changes the prime numbers generated from a given seed. We believe this is the final breaking change for olivevdf.
...
- calccrypto/uint128_t was updated in the Windows olivepos implementation. Chiapos required some changes its build process to support MacOS ARM64.
...
- Adding Click changed the behavior of `olive keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.olive.net via https, for example.
- The difficulty adjustment calculation was simplified.
- All of the olive sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
...
- Various typos and style clean ups were made to the Click CLI implementation. `olive -upnp f` was added to disable uPnP.
- `olive plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major olivelisp changes. TXCH from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- A new RPC api: /push_tx. Using this RPC, you can spend custom olivelisp programs. You need to make a SpendBundle, which includes the puzzle reveal (olivelisp), a solution (olivelisp) and a signature.
- You can now use the RPC apis to query the mempool.
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/olive-blockchain).
- You can obtain a new wallet receive address on the command line with `olive wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
- You will now see Your Harvester Network in the GUI even if you have no plots.

### Changed

- All olivelisp opcodes have been renumbered. This should be the last major breaking change for olivelisp and the clvm. There are a couple minor enhancements still needed for mainnet launch, but they may or may not require minor breaking changes. We will be restarting testnet chains on a mostly weekly basis either way.
- Node batch syncing performance was increased, and it now avoids re-validating blocks that node had already validated.
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/Chia-Network/olive-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `olive -h` and `olive SUBCOMMAND -h` can be your guide.
...
- `olive plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `olive netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `olive init` only migrates release candidate directories. The versioned sub directories under `~/olive` will be going away before mainnet.
...
- `olive configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Chia-Network/olive-blockchain/releases/tag/1.0rc1) was well.
...
- Use 'olive wallet get_transactions' in the command line to see your transactions.
- 'olive wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [Chia Blockchain GUI](https://crowdin.com/project/olive-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/Chia-Network/olive-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/Chia-Network/olive-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `olive plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'olive wallet send' now takes in TXCH or XCH as units instead of mojos.
...
- You can now send and receive transactions with the command line. Try `olive wallet -h` to learn more. Also, `olive wallet` now requires a third argument of `show`, therefor you will use `olive wallet show` to see your wallet balance.
...
- We build both MacOS x86_64 and MacOS universal wheels for olivepos, olivevdf, blpsy, and olivebip158 in Python 3.9. The universal build allows M1 Macs to run these dependencies in ARM64 native mode.
...
- The GUI is now in a separate submodule repository from olive-blockchain, [olive-blockchain-gui](https://github.com/Chia-Network/olive-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `olive-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `olive wallet`, was incorrectly displaying TXCH balances as if they were Coloured Coins.
...
- `olive plots check -l` will list all duplicate plot IDs found on the machine. Thanks very much for this PR @eFishCent.
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Chia-Network/olive-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- There was a potential bug in Python 3.9.0 that required us to update blspy, olivepos, olivevdf, and olivebip158.
...
- `olive plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- We have removed the '-r' flag from `olive` as it was being overridden in most cases by the `-r` for restart flag to `olive start`. Use `olive --root-path` instead.
- `olive -h` now recommends `olive netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `olive show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `olive configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `olive plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `olive plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `olive show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `olive show` handles sub block 0 better.
...
- `olive plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `olive plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `olive show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `olive configure --enable-upnp`.
...
- `olive configure` now lets you enable or disable uPnP.
...
- `olive netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `olive show` of the GUI use `olive netspace -d 1000` as `olive netspace` defaults to `-d 192` which is one hour.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [olive.net](https://olive.net/). Among the improvements this gives the Chia blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- We use a faster hash to prime function for olivevdf from the current release of gmp-6.2.1 which we have upgraded olivevdf and blspy to support.
- There is a new cli command - `olive configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `olive configure -h`. Note that if olive services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- `olive show -s` has been refactored to support the new consensus.
- `olive netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.olive/ directory to speed up total testing time.
- Changes were made to olivepos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run olive-blockchain.
...
- `olive show -s` now is less brain dead when a node is initially starting to sync.
...
- We have bitfield optimized phase 2 of plotting. There is only about a 1% increase in speed from this change but there is a 12% decrease in writes with a penalty of 3% more reads. More details in [PR 120](https://github.com/Chia-Network/olivepos/pull/120). Note that some sorts in phase 2 and phase 3 will now appear "out of order" and that is now expected behavior.
- Partial support for Python 3.9. That includes new versions of Chia dependencies like olivebip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/Chia-Network/olive-blockchain/issues/500).
...
- In the GUI there was [a regression](https://github.com/Chia-Network/olive-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/Chia-Network/olive-blockchain/issues/485) where log level could no longer be set by service.
...
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Chia-Network/olive-blockchain/tree/dev/electron-react/src/locales) of the olive-blockchain repository.
...
- Updated to olivepos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [olivepos release notes](https://github.com/Chia-Network/olivepos/releases/tag/0.12.32).
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Chia Network at backup.olive.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Chia backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.olive.net, your own installation, or a third party's version of it.
...
- There was a regression in Chia Proof of Space ([olivepos](https://github.com/Chia-Network/olivepos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/Chia-Network/olive-blockchain/wiki) and placed in [INSTALL](https://github.com/Chia-Network/olive-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/Chia-Network/olive-blockchain/wiki/Quick-Start-Guide)
...
- Chia start commands like `olive start farmer` and `olive stop node` now keep track of process IDs in a run/ directory in your configuration directory. `olive stop` is unlikely to work on Windows native for now. If `olive start -r node` doesn't work you can force the run/ directory to be reset with `olive start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/Chia-Network/olive-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/Chia-Network/olive-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- There is a new command `olive init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `olive-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Chia-Network/olive-blockchain/wiki/Updating-beta-software). If you've set `$OLIVE_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We have revamped the olive management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `olive-start-farmer &`. The [README.md](https://github.com/Chia-Network/olive-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- The libraries olive-pos, olive-fast-vdf, and olive-bip-158 have been moved to their own repositories: [olivepos](https://github.com/Chia-Network/olivepos), [olivevdf](https://github.com/Chia-Network/olivevdf), and [chaibip158](https://github.com/Chia-Network/olivebip158). They are brought in by olive-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Chia-Network/bls-signatures).
...
- Chia fullnode, farmer and harvester now default to logging to olive.log in the olive-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
[unreleased]: https://github.com/Chia-Network/olive-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/Chia-Network/olive-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/Chia-Network/olive-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/Chia-Network/olive-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/Chia-Network/olive-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/Chia-Network/olive-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/Chia-Network/olive-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/Chia-Network/olive-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/Chia-Network/olive-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/Chia-Network/olive-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/Chia-Network/olive-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/Chia-Network/olive-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/Chia-Network/olive-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/Chia-Network/olive-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/Chia-Network/olive-blockchain/releases/tag/Alpha-1.0
