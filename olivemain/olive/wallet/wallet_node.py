...
from olive.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from olive.protocols import wallet_protocol
from olive.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from olive.protocols.protocol_message_types import ProtocolMessageTypes
from olive.protocols.wallet_protocol import (
...
from olive.util.ints import uint32, uint128
from olive.util.keychain import Keychain
from olive.util.lru_cache import LRUCache
from olive.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from olive.wallet.settings.settings_objects import BackupInitialized
...
from olive.wallet.wallet_blockchain import ReceiveBlockResult
from olive.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'olive keys' program.")
            return None
    ...
    async def get_additions(
        self, peer: WSoliveConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSoliveConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
