...
import oliveEnvironment from '../util/oliveEnvironment';
import oliveConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!oliveEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    oliveConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !oliveConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'olive Blockchain Website' }),
            click: () => {
              openExternal(
                'https://oliveblockchain.co',
              );
            },
          },
          {
