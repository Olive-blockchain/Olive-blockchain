...
import { AlertDialog } from '@olive/core';
import {
  Typography,
  Button,
  Box,
...
import { olive_to_dog } from '../../../util/olive';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      oliveper_input.value === '' ||
      Number(oliveper_input.value) === 0 ||
      !Number(oliveper_input.value) ||
      isNaN(Number(oliveper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const oliveper = olive_to_dog(oliveper_input.value);
    const oliveper_value = Number.parseInt(Number(oliveper));
    const userpubkey = userpubkey_input.value;
    const amount = olive_to_dog(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = olive_to_dog(fee_input.value);
