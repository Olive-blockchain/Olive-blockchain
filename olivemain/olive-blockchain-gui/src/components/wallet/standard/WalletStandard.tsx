...
import { /* dog_to_olive_string, */ olive_to_dog } from '../../../util/olive';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {dog_to_olive_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    const amountValue = Number.parseFloat(olive_to_dog(amount));
    const feeValue = Number.parseFloat(olive_to_dog(fee));
