...
from olive.ssl.create_ssl import generate_ca_signed_cert, get_olive_ca_crt_key, make_ca_cert
from olive.util.bech32m import encode_puzzle_hash
from olive.util.config import (
    create_default_olive_config,
    initial_config_file,
    load_config,
    save_config,
...
from olive.util.keychain import Keychain
from olive.util.path import mkdir
from olive.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'olive keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "xol_target_address" not in config["farmer"]
    stop_searching_for_pool = "xol_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("xol_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("xol_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "xol_target_address" not in config["farmer"]:
        print(f"Setting the xol destination address for coinbase fees reward to {all_targets[0]}")
        config["farmer"]["xol_target_address"] = all_targets[0]
    elif config["farmer"]["xol_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['xol_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "xol_target_address" not in config["pool"]:
        print(f"Setting the xol destination address for coinbase reward to {all_targets[0]}")
        config["pool"]["xol_target_address"] = all_targets[0]
    elif config["pool"]["xol_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['xol_target_address']} with {all_targets[0]}"
        )
...
def create_all_ssl(root: Path):
    ...
    olive_ca_crt, olive_ca_key = get_olive_ca_crt_key()
    olive_ca_crt_path = ca_dir / "olive_ca.crt"
    olive_ca_key_path = ca_dir / "olive_ca.key"
    olive_ca_crt_path.write_bytes(olive_ca_crt)
    olive_ca_key_path.write_bytes(olive_ca_key)
    ...
    olive_ca_crt, olive_ca_key = get_olive_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, olive_ca_crt, olive_ca_key, False, overwrite=False)
...
def olive_version_number() -> Tuple[str, str, str, str]:
...
def olive_minor_release_number():
    res = int(olive_version_number()[2])
    print(f"Install release number: {res}")
    return res


def olive_full_version_str() -> str:
    major, minor, patch, dev = olive_version_number()
...
def olive_init(root_path: Path):
    ...
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if CHIA_ROOT is set, or if user has run olive init twice
        # before a new update.
        check_keys(root_path)
        print(f"{root_path} already exists, no migration action taken")
        return -1

    create_default_olive_config(root_path)
    create_all_ssl(root_path)
    check_keys(root_path)
    print("")
    print("To see your keys, run 'olive keys show --show-mnemonic-seed'")
