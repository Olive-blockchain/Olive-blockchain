...
from olive.util.misc import format_minutes
from olive.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total olive farmed: {amounts['farmed_amount'] / units['olive']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['olive']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['olive']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'olive start wallet' and 'olive wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'olive wallet show'")
    else:
        print("Note: log into your key using 'olive wallet show' to see rewards for each key")
