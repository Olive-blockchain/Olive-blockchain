...
from olive.cmds.configure import configure_cmd
from olive.cmds.farm import farm_cmd
from olive.cmds.init import init_cmd
from olive.cmds.keys import keys_cmd
from olive.cmds.netspace import netspace_cmd
from olive.cmds.plots import plots_cmd
from olive.cmds.show import show_cmd
...
@click.group(
    help=f"\n  Manage olive blockchain infrastructure ({__version__})\n",
    epilog="Try 'olive start node', 'olive netspace -d 192', or 'olive show -s'",
...
@cli.command("version", short_help="Show olive version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs olive daemon")
@click.pass_context
def run_daemon_cmd(ctx: click.Context) -> None:
    from olive.daemon.server import async_run_daemon
    import asyncio
