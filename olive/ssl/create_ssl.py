...
def get_olive_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "olive_ca.crt")
    key = pkg_resources.resource_string(__name__, "olive_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("olivenetwork.org")]),
            critical=False,
        )
...
def main():
    return make_ca_cert(Path("./olive_ca.crt"), Path("./olive_ca.key"))
