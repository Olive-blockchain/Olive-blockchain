...
from olive.util.keychain import bytes_to_mnemonic, generate_mnemonic
from olive.util.path import path_from_root
from olive.util.ws_message import WsRpcMessage, create_payload_dict
from olive.wallet.cc_wallet.cc_wallet import CCWallet
from olive.wallet.derive_keys import master_sk_to_singleton_owner_sk
...
from olive.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from olive.wallet.util.trade_utils import trade_record_to_dict
from olive.wallet.util.transaction_type import TransactionType
from olive.wallet.util.wallet_types import WalletType
from olive.wallet.wallet_info import WalletInfo
from olive.wallet.wallet_node import WalletNode
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "olive_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "olive_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xol_target_address")
        pool_target = config["pool"].get("xol_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from olive.pools.pool_wallet_info import initial_pool_state_from_dict
