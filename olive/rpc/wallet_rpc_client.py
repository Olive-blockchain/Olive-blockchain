...
from olive.pools.pool_wallet_info import PoolWalletInfo
...
class WalletRpcClient(RpcClient):
    """
    Client to Olive RPC, connects to a local wallet. Uses HTTP/JSON, and converts back from
    ...
    async def log_in(self, fingerprint: int) -> Dict:
        try:
            return await self.fetch(
                "log_in",
                {"host": "https://backup.oliveblockchain.co", "fingerprint": fingerprint, "type": "start"},
            )
    ...
    async def log_in_and_restore(self, fingerprint: int, file_path) -> Dict:
        try:
            return await self.fetch(
                "log_in",
                {
                    "host": "https://backup.oliveblockchain.co",
    ...
    async def log_in_and_skip(self, fingerprint: int) -> Dict:
        try:
            return await self.fetch(
                "log_in",
                {"host": "https://backup.oliveblockchain.co", "fingerprint": fingerprint, "type": "skip"},
            )
        except ValueError as e:
            return e.args[0]
