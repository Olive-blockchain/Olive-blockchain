...
from olive.types.coin_solution import CoinSolution
...
from olive.wallet.cc_wallet import cc_utils
from olive.wallet.cc_wallet.cc_utils import CC_MOD, SpendableCC, spend_bundle_for_spendable_ccs, uncurry_cc
from olive.wallet.cc_wallet.cc_wallet import CCWallet
from olive.wallet.puzzles.genesis_by_coin_id_with_0 import genesis_coin_id_for_genesis_coin_checker
from olive.wallet.trade_record import TradeRecord
from olive.wallet.trading.trade_status import TradeStatus
from olive.wallet.trading.trade_store import TradeStore
from olive.wallet.transaction_record import TransactionRecord
from olive.wallet.util.trade_utils import (
...
from olive.wallet.util.transaction_type import TransactionType
from olive.wallet.util.wallet_types import WalletType
from olive.wallet.wallet import Wallet
from olive.wallet.wallet_coin_record import WalletCoinRecord

# from olive.wallet.cc_wallet.debug_spend_bundle import debug_spend_bundle
            ...
            for id in offer.keys():
                ...
                if isinstance(wallet, CCWallet):
                    ...
                    if spend_bundle is None:
                        ...
                        to_exclude = spend_bundle.removals()
                    new_spend_bundle = await wallet.create_spend_bundle_relative_olive(amount, to_exclude)
                else:
    ...
    async def respond_to_offer(self, file_path: Path) -> Tuple[bool, Optional[TradeRecord], Optional[str]]:
        ...
        for coinsol in offer_spend_bundle.coin_solutions:
            ...
            if r:
                ...
                if coinsol.coin in [record.coin for record in unspent]:
                    return False, None, "can't respond to own offer"
                if olive_discrepancy is None:
                    olive_discrepancy = get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                else:
                    olive_discrepancy += get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                coinsols.append(coinsol)

        olive_spend_bundle: Optional[SpendBundle] = None
        if olive_discrepancy is not None:
            olive_spend_bundle = await self.wallet_state_manager.main_wallet.create_spend_bundle_relative_olive(
                olive_discrepancy, []
            )
            if olive_spend_bundle is not None:
                for coinsol in coinsols:
                    olive_spend_bundle.coin_solutions.append(coinsol)
        ...
        for colour in cc_discrepancies.keys():
            if cc_discrepancies[colour] < 0:
                ...
                if olive_spend_bundle is None:
                    to_exclude: List = []
                else:
                    to_exclude = olive_spend_bundle.removals()
                ...
                if my_cc_spends is None or my_cc_spends == set():
                    ...
                    if zero_spend_bundle is None:
                        return (
                            False,
                            None,
                            "Unable to generate zero value coin. Confirm that you have olive available",
                        )
        ...
        now = uint64(int(time.time()))
        if olive_spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, olive_spend_bundle])
            # debug_spend_bundle(spend_bundle)
            if olive_discrepancy < 0:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
                    created_at_time=now,
                    to_puzzle_hash=token_bytes(),
                    amount=uint64(abs(olive_discrepancy)),
                    fee_amount=uint64(0),
                    confirmed=False,
                    sent=uint32(10),
                    spend_bundle=olive_spend_bundle,
                    additions=olive_spend_bundle.additions(),
                    removals=olive_spend_bundle.removals(),
                    wallet_id=uint32(1),
                    sent_to=[],
                    trade_id=std_hash(spend_bundle.name() + bytes(now)),
                    type=uint32(TransactionType.OUTGOING_TRADE.value),
                    name=olive_spend_bundle.name(),
                )
            else:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
