...
export default function Block() {
  ...
  const poolReward = mojo_to_olive(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_olive(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const oliveFees = blockRecord.fees
    ? mojo_to_olive(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.oliveexplorer.org/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.oliveexplorer.org/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: oliveFees ? `${oliveFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
