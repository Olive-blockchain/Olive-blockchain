  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of olive in the blockchain at the current
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      dispatch(
        openDialog(
          <AlertDialog>
            <Trans>
              Error: Cannot send olive to coloured address. Please enter a olive
              address.
            </Trans>
          </AlertDialog>,
        ),
    ...
    if (address.slice(0, 12) === 'olive_addr://') {
    ...
    const amountValue = Number.parseFloat(olive_to_mojo(amount));
    const feeValue = Number.parseFloat(olive_to_mojo(fee));
