...
import { AlertDialog, Card, Flex } from '@olive/core';
import {
  send_transaction,
  rl_set_user_info_action,
} from '../../../modules/message';
import { mojo_to_olive_string, olive_to_mojo } from '../../../util/olive';
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const hexoleck = /[\da-f]+$/gi;

    if (!hexoleck.test(ip_val) || ip_val.value === '') {
    ...
    const oliveper_input = ip_parsed.limit;
    ...
    const oliveper_value = Number.parseInt(Number(oliveper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (olive per interval):{' '}
                  {mojo_to_olive_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_olive_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = olive_to_mojo(amount_input.value);
    const fee = olive_to_mojo(fee_input.value);
